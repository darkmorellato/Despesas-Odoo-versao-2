...
async function main() {
  ...
  // 2. Insert the exact items from FIXED_NOTIFICATIONS_DEFAULT
  console.log(`Gravando os ${FIXED_NOTIFICATIONS_DEFAULT.length} pagamentos fixos atualizados...`);
  ...
  console.log(`✅ Todos os ${FIXED_NOTIFICATIONS_DEFAULT.length} pagamentos fixos gravados no Firestore!`);
  ...
  for (const [key, value] of Object.entries(currentChecks)) {
    ...
    newChecks[key] = value as boolean;
  }
